...
##Read and clean other data

datafile <- rbind(readRDS("Data201908082.rds"),
                  readRDS("Data20190808.rds"),
                  readRDS("Data20190908.rds"),
                  readRDS("Data20190912.rds"),
                  readRDS("Data20190912.rds"),
                  readRDS("Data20190916.rds"),
                  readRDS("Data20190919.rds"),
                  readRDS("Data20190920.rds"))

datafileDedupe <- datafile[duplicated(datafile),] 
datafileDate <- datafileDedupe %>%
    mutate(Date = as.POSIXct(round_any(as.numeric(
        Date)+60,300, f = floor)
...
dtAll <- rbind(dt,datafileDate)

dtAll2 <- dtAll[order(dtAll$Date, dtAll$Ride, dtAll$Wait,
                      dtAll$HTML_ID, decreasing=TRUE),]
       
dtAll3 <- dtAll2[!duplicated(dtAll2 %>% select(Date,Ride)),]

dtAll4 <- dtAll3[order(dtAll3$Date, dtAll3$Ride),] %>%
    filter(!Ride=="Coming Soon") %>% filter(!Ride=="Weather Delay")
...
for (j in 1:NumRows) {
    ...
    if (j==NumRows){
            dtAllTest <- dtAll4[((j-1)*1000+1):length(dtAll4$Ride),]
        } else {
            dtAllTest <- dtAll4[((j-1)*1000+1):(j*1000),]   
        }
    
valueList <- paste0(apply(dtAllTest, 1, function(x) paste0("('", 
